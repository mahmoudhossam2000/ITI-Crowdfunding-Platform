...
async function loadPledges() {
  ...
  pledges.forEach(async (pledge) => {
    const campaignRes = await fetch(
      `http://localhost:3000/campaigns/${pledge.campaignId}`
    );
          ...
          <div class="row g-0">
            ...
            <div class="col-md-8">
              <div class="card-body d-flex flex-column">
                ...
                <p class="pledge-info mb-1"><strong>Pledged:</strong> $${
                  pledge.amount
                }</p>
                <p class="pledge-info mb-1"><strong>Date:</strong> ${new Date(
                  pledge.timestamp
                ).toLocaleDateString()}</p>
                <div class="mt-auto">
                  <a href="/campaign.html?id=${
                    campaign.id
                  }" class="btn btn-outline-primary btn-sm">View Campaign</a>
                </div>
              </div>
            </div>
          </div>
...
async function loadCategoriesAndCampaigns() {
  ...
  try {
    ...
    campaignList.innerHTML =
      '<p class="text-danger">Failed to load campaigns.</p>';
  }
}
