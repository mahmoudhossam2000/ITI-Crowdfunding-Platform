...
// Load pending campaigners
async function loadPendingUsers() {
    try {
        const response = await fetch(`${config.API_URL}${config.ROUTES.USERS}?role=${config.ROLES.CAMPAIGNER}&status=${config.STATUS.PENDING}`);
        ...
        tbody.innerHTML = users.map(user => `
            <tr>
                ...
                <td><span class="badge bg-warning">${config.STATUS.PENDING}</span></td>
                <td>
...
async function approveUser(userId) {
    try {
        const response = await fetch(`${config.API_URL}${config.ROUTES.USERS}/${userId}`, {
            method: 'PATCH',
            headers: {
